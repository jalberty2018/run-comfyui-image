...
The [model provisioning guides](ComfyUI_image_provisioning.md) cover Flux.1,
Flux.2, Z-Image, Qwen Image and Edit, ERNIE Image, JoyAI, Hunyuan, Chroma1,
Ovis and Mage-flow, together with segmentation and upscaling models. Review
each artifact's model card, license file and referenced original publisher
terms separately, including text encoders, VAEs, LoRAs and derivatives.
Different releases within a model family may have different terms; do not
assume that one variant's permissions apply to another. A custom node's
software license does not establish the license of the weights it downloads.
